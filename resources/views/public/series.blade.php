    ...
    <main class="catalog-page">
        <div class="container">
            ...
            <div class="page-header">
                <h2 class="page-title">Séries <span>({{ $series->total() }} títulos)</span></h2>
            ...
            <div class="catalog-grid">
                @forelse($series as $item)
                    ...
                    @endphp
                    @php
                        $itemUrl = $item->slug ? route('series.show', $item->slug) : 'javascript:void(0)';
                    @endphp
            ...
            @if($series->hasPages())
                <div class="pagination">
                    ...
                    @else
                        <a href="{{ $series->previousPageUrl() }}" class="page-btn"><i class="fas fa-chevron-left"></i></a>
                    @endif
                    ...
                    @if($start > 1)
                        <a href="{{ $series->url(1) }}" class="page-btn">1</a>
                        @if($start > 2) <span style="color: var(--text-muted);">...</span> @endif
                    @endif

                    @for($i = $start; $i <= $end; $i++)
                        <a href="{{ $series->url($i) }}" class="page-btn {{ $i == $series->currentPage() ? 'active' : '' }}">
                            {{ $i }}
                        </a>
                    @endfor

                    @if($end < $series->lastPage())
                        @if($end < $series->lastPage() - 1) <span style="color: var(--text-muted);">...</span> @endif
                        <a href="{{ $series->url($series->lastPage()) }}" class="page-btn">{{ $series->lastPage() }}</a>
                    @endif

                    @if($series->hasMorePages())
                        <a href="{{ $series->nextPageUrl() }}" class="page-btn"><i class="fas fa-chevron-right"></i></a>
                    @else
